...
src_prepare() {
	...
	if has_version ">=media-libs/freetype-2.5.1" ; then
		sed -i -e 's/#include <freetype\/ftoutln.h>/#include FT_OUTLINE_H/' fbsplash/*.h
		sed -i -e 's/#include <freetype\/ttnameid.h>/#include FT_TRUETYPE_IDS_H/' fbsplash/*.h
		sed -i -e 's/#include <freetype\/ftoutln.h>/#include FT_OUTLINE_H/' fbsplash/*.c
		sed -i -e 's/#include <freetype\/ttnameid.h>/#include FT_TRUETYPE_IDS_H/' fbsplash/*.c
	fi
}

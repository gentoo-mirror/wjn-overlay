...
LICENSE="BSD sndfile? ( GPL-2+ ) filewriter? ( GPL-2+ )
	libnotify? ( GPL-3+ ) pulseaudio? ( GPL-2+ ) spectrum? ( GPL-2+ )"
SLOT="0"
KEYWORDS="alpha amd64 ~arm hppa ppc ppc64 sparc 
	x86 ~x86-fbsd ~x86-freebsd ~x86-interix ~amd64-linux ~x86-linux"
IUSE="aac adplug alsa bs2b cdda cue ffmpeg +filewriter flac gnome jack 
	lame libnotify libsamplerate lirc midi mms mp3 nls pulseaudio 
